...
def update_arm_swing(node: Node, delta_time: float, time_since_start: float) -> None:
    ...
    node.rotation[2] = node.swing_sign * (100.0 * bounce)
...
def build_boneco(cube_mesh, skin_material: Material,
                  shirt_material: Material, pants_material: Material) -> Node:
    '''
...
if __name__ == "__main__":
    ...
    floor_color = Texture.load_file(
        "assets/Rock035_1K-JPG/Rock035_1K-JPG_Color.jpg", srgb=True, drop_alpha=True)
    floor_roughness = Texture.load_file(
        "assets/Rock035_1K-JPG/Rock035_1K-JPG_Roughness.jpg", drop_alpha=True)
    ...
    floor_material = Material(shader)
    floor_material.set_texture(0, "baseColorTexture", floor_color)
    floor_material.set_texture(1, "metallicTexture", black_r)
    floor_material.set_texture(2, "roughnessTexture", floor_roughness)
    floor_material.set_uniform("tiling", 6.0)
    ...
    boneco = build_boneco(cube_mesh, skin_material, shirt_material, pants_material)
    ...
    disco_ball.translation = np.array([0.0, 3.3, -5.0])
    ...
    # Piso
    floor = Node("floor")
    floor.translation = np.array([0.0, -0.05, -5.0])
    floor.scale = np.array([12.0, 0.1, 12.0])
    floor.render_data["mesh"] = cube_mesh
    floor.render_data["material"] = floor_material
    runtime.scene.add_child(floor)

    # Câmera
    runtime.camera.translation = np.array([0.0, 1.4, 1.2], np.float64)
